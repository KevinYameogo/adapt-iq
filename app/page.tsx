...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hi! I'm the Adapt IQ Assistant. How can I help you optimize your presentation today?" }
  ]);
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "That sounds like a great project! I'm currently in 'Demo Mode,' but I can certainly help you brainstorm slide layouts here." }
      ]);
      setIsTyping(false);
    }, 1500);
  };
  ...
  const handleStartAction = () => {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "🚀 Adapt IQ is now online. I've analyzed your current slide deck. How should we proceed?" }
      ]);
    }, 2000);
  };
  ...
  return (
    <div className="relative min-h-screen bg-white font-sans text-slate-900 overflow-x-hidden">
      ...
      <main className="relative">
        {/* SECTION 1: HERO */}
        <section id="home" className="relative w-full aspect-video bg-black overflow-hidden flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-20">
            <span className="text-6xl text-white font-black italic border-b-4 border-white">VIDEO</span>
          </div>
        </section>
        ...
        <div className="p-6 space-y-20">
          ...
          <section id="solutions" className="space-y-10 py-6 scroll-mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-2">
                <ServiceCard emoji="📝" title="Note-to-Slide" desc="Turn rough ideas into polished structured slides instantly." />
                <ServiceCard emoji="📊" title="Smart Summaries" desc="Generate comparisons and pros/cons directly inside PowerPoint." />
                <ServiceCard emoji="🔍" title="Research AI" desc="Real-time fact checking and data sourcing for every slide." />
            </div>
          </section>

          {/* SECTION 5: FOOTER & START BUTTON */}
          <footer id="contact" className="-mx-6 -mb-6 bg-slate-950 p-10 text-white rounded-t-[3.5rem] relative z-10 shadow-2xl scroll-mt-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12 text-left">
              ...
              <FooterColumn title="Resources" links={["FAQs", "Documentation"]} />
            ...
            <div className="mt-12 pt-8 border-t border-white/5 flex flex-col items-center relative">
               ...
               <button 
                 ...
                 className={`group relative flex items-center gap-3 bg-indigo-600 px-10 py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] text-white transition-all active:scale-95 shadow-2xl shadow-indigo-500/20 z-10 overflow-hidden ${isLaunching ? 'cursor-not-allowed opacity-90' : 'hover:bg-indigo-500'}`}
               >
            ...
            <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 mt-12">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">ADAPT IQ © 2026</span>
      ...
      {isChatOpen && (
        <div className="fixed bottom-24 right-6 w-72 bg-white rounded-3xl shadow-2xl z-[100] border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-indigo-600 p-4 text-white flex justify-between items-center">
            <span className="font-bold text-[10px] uppercase tracking-widest">Adapt IQ Support</span>
            <button onClick={() => setIsChatOpen(false)} className="text-2xl font-light">×</button>
          </div>
          ...
          <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-100 bg-white flex gap-2">
            ...
            />
            <button type="submit" className="bg-indigo-600 text-white px-3 py-1 rounded-xl text-[10px] font-bold">Send</button>
          </form>
        </div>
      )}
    ...
    return (
      <div className="space-y-3">
        <div className="w-full aspect-[4/3] bg-slate-50 rounded-[1.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center group relative overflow-hidden transition-all hover:border-indigo-300">
          <span className="text-3xl mb-1 group-hover:scale-110 transition-transform">{emoji}</span>
          <span className="text-[7px] font-bold text-slate-300 uppercase tracking-widest text-center px-4">Interface Preview</span>
        </div>
...
function NavIcon({ icon, label, sub }: { icon: string; label: string; sub: string; }) {
    return (
...
function FooterColumn({ title, links }: { title: string; links: string[] }) {
    return (
      <div className="flex flex-col gap-2">
        <h4 className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-1">{title}</h4>
        {links.map((link) => (
          <a key={link} href="#" className="text-xs text-slate-300 hover:text-white transition-colors">
            {link}
          </a>
        ))}
      </div>
